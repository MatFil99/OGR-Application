...
def recognize_topology(preprocessed, vertices_img, vertices_list, user_params: UserParams):
    """
    """
    closed_img = cv.morphologyEx(preprocessed, cv.MORPH_CLOSE, Kernel.k3)
...
def angles_of_line_endpoints(cnt, pt1, pt2):
    ...
    neighbors_pt2 = get_nearby_points(pt2, cnt, 10, 15)
...
def angle_of_tangent(pt, neighbors):
    ...
    if max_diff > 180:
        ...
        np.where(angles_list < 0, angles_list+360, angles_list) #
...
def are_parts_of_line_or_curve(line1, line2, angle_threshold=Options.ANGLE_THRESHOLD, xy_threshold=Options.XY_THRESHOLD):
    ...
    angle1 = line_angle(line1.pt1, line1.pt2)
    angle2 = line_angle(line2.pt1, line2.pt2)

    # angle_diff = angle1 - angle2 if angle1*angle2>0 else max(angle1, angle2)-(180 - min(angle1, angle2))
    angle_diff_endpts = min(abs(angle1 - angle2), abs(max(angle1,angle2) - (180+min(angle1,angle2))))
    ...
    is_c = is_curve(line1.angle_pt1, line1.angle_pt2, angle1)
    ...
    if is_c:
        ...
        if -angle_threshold <= angle_diff_endpts <= angle_threshold:
            
            if -45 <= angle1 <= 45:
                y3_p = P1[1] + np.tan(np.radians(angle1)) * (P2[0] - P1[0]) # P2.y + tan(angle1) * (P3.x-P2.x)
                delta = np.abs(P2[1] - y3_p)
            else:
                x3_p = P1[0] + (P2[1] - P1[1])/np.tan(np.radians(angle1)) # P1.x + (P2.y - P1.y) / tan(angle1)
                delta = min(delta, np.abs(P2[0] - x3_p))

    if P1_angle == Options.UNDEFINED_ANGLE or delta > xy_threshold:
...
def link_vertices(vertices_list, lines):
    ...
    for vidx in range(0, len(vertices_list)):
        v = vertices_list[vidx]
        nearby_lines = nearby_lines_of_vertex(v, lines)
        for lidx in nearby_lines:
            incident_matrix[vidx, lidx] = 1
...
def nearby_lines_of_vertex(vertex, lines):
    nearby_lines = []
    for i in range(0,len(lines)):
        v_center = [vertex.x, vertex.y]
        line = lines[i]
        if distance_2P(v_center, line.pt1) < vertex.r * (1 + Options.AREA_VERTEX_EDGE) \
            or distance_2P(v_center, line.pt2) < vertex.r * (1 + Options.AREA_VERTEX_EDGE):
            nearby_lines.append(i) # the edge is connected to the vertex
    return nearby_lines
