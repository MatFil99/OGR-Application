...
def recognize_topology(preprocessed, vertices_img, vertices_list, user_params: UserParams):
    ...
    linked_lines = link_nearby_lines(lines, vertices_list)
...
def fit_line(cnt):
    """
    find endpoints of line or curve
    """
...
def link_nearby_lines(lines, vertices):
    """
...
def vector2line_angle(vangle):
    """
    convert vector angle to line angle - from range (-PI, PI] to (-PI/2, PI/2]
    """
    if -90 <= vangle <= 90:
        return vangle
    elif vangle < -90:
...
def line_angle(pt1, pt2):
    ...
    returns values (-PI/2, PI/2] (ignores direction of vector)
...
def link_vertices(vertices_list, lines):
    """
    return recognized vertices and edges, and other lines which are not edges
    """
    linked_vertices = []
...
def nearby_vertices_of_line(line, vertices):
    ...
    best_dis1 = Options.MAX_VERTEX_RADIUS * Options.AREA_VERTEX_EDGE
    best_dis2 = Options.MAX_VERTEX_RADIUS * Options.AREA_VERTEX_EDGE


    for vidx in range(0, len(vertices)):
        v = vertices[vidx]

        max_dis1 = v.r * Options.AREA_VERTEX_EDGE
        max_dis2 = v.r * Options.AREA_VERTEX_EDGE
